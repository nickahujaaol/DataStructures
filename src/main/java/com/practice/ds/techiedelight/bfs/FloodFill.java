...
public class FloodFill {
    ...
    private static void floodFill(char[][] matrix, int targetRow, int targetCol, char replacementColor) {
        ...
        matrix[targetRow][targetCol] = replacementColor;
        ...
        while (!queue.isEmpty()) {
            ...
            for(int i = 0; i < row.length; i++) {
                if(isValid(matrix, removedPair.x + row[i], removedPair.y + col[i], 'X', visited)) {
                    visited[removedPair.x + row[i]][removedPair.y + col[i]] = true;
                    matrix[removedPair.x + row[i]][removedPair.y + col[i]] = replacementColor;
